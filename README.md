...
##### **Regression Methods:**
The next supervised learning method that was used is several regression techniques to attempt to predict the actual profit generated by the movie using certain factors. We used two techniques. Since the dataset had both categorical as well as continuous features we best went wit the multi-layer perceptron neural network and the gradient boosting method.

We used the overall dataset (after preprocessing), and broke it up into an X and y dataset. The X dataset consisted of all the preprocessed data, except the features titled ‘revenue’, ‘profit’, ‘title’, and ‘id’. Revenue and profit were dropped, because these are essentially our target. Title and ID were removed, because they are strings that are irrelevant to generating decision trees. The y dataset consisted of simply our target variable, ‘profit’. From here, we further broke this data down into training (80%) and test (20%) sets.
The mlp neural network used two hidden layers with 50 and 25 neurons respectively in those layers. The respective models were then fit and the mean squared errors were assessed. Further more, since there were over 400 features and worried that the complexity of the model could induce overfitting, for both of the models we implemented the forward feature selection that would choose atmost 20 best features to predict the model. The mean squared error was the deciding factor for the feature selection.
